(function(){
    var pushcrewConfig = {
        ...
        serverSyncUrl: "https://pushcrewlocal.com",
        subdomain: "go3",
        ...
        jsAPIVersion: 4,
        segmentations: [],
        servicesServerURL: "http://dacdn.pushcrewlocal.com",
        style: {
            url: "https://pushcrewlocal.com/http-v4/css/httpFront-v4.css?v=1",
            type: "safari",
            iconUrl: "https://cdn.pushcrew.com/img/logos/7d6b890795fb42cad6db2c90045a69d4/924e84da-24c8-49ef-ae9d-9e0554ab6997.png",
            allowTextChildWindow: "",
            offerTextChildWindow: "Get updates and offers from wingify by subscribing to push notifications.",
            ...
            chromeTitle: "ct1",
            chromeSubtitle: "ct2",
            safariTitle: "The website “wingify” would like to send you push notifications.",
...
var declarations, detectPrivateMode, globalConfig, storageUtil, jquery, customizejQuery, initAPI, api, pushSubscription, jstz, manageSubscription, httpChildMain, vwoUtils_cookies_CookiesEventEnum, vwoUtils_triggerEvent, vwoUtils_tld, vwoUtils_cookies_cookies, vwoUtils_referrer, vwoUtils_segmentation, src_httpChild_without_tracker;
...
detectPrivateMode = function () {
  ...
  return {
    retry: retry,
    isIE10OrLater: isIE10OrLater,
    detectPrivateMode: detectPrivateMode
  };
}();
globalConfig = function () {
  var _utils, _internalq, apiFailureQueue, components, APIReadyState, _pushcrew, pushcrew, pushAPI;
  apiFailureQueue = [];
  _utils = {};
  _internalq = [];
  components = {};
      ...
      for (var i = 0; i < arguments.length; i++)
        try {
          if (pushcrew.isAPIReady === APIReadyState.UNINITIALIZED) {
            if (arguments[i][0] === 'APIReady') {
              if (arguments[i][2] !== undefined && typeof arguments[i][2] === 'function') {
          ...
          } else if (pushcrew.isAPIReady === APIReadyState.FAILED) {
            if (arguments[i][0] === 'APIReady') {
              if (arguments[i][2] !== undefined && typeof arguments[i][2] === 'function') {
                arguments[i][2](_pushcrew.apiFailureReasons);
              }
            }
          } else if (pushcrew.isAPIReady === APIReadyState.READY) {
            if (arguments[i][2] !== undefined) {
              _pushcrew[arguments[i][0]](arguments[i][1], arguments[i][2]);
            } else {
              _pushcrew[arguments[i][0]](arguments[i][1], null);
...
storageUtil = function (globalConfig) {
  globalConfig.pushAPI.storageUtil = function () {
    ...
    return {
      ...
      getAllCookiesWithMatchRegex: function (str) {
        var pairs = document.cookie.split(';'), cookies = {};
      ...
      getAllStorageItemsWithMatchRegex: function (str) {
        var storageItems = {}, keys = Object.keys(localStorage), i = keys.length;
        while (i--) {
          if (keys[i].startsWith(str)) {
            storageItems[keys[i]] = localStorage.getItem(keys[i]);
          }
        }
        return storageItems;
      },
      ...
      */
      getFromStorage: function (cname) {
        if (localStorage.getItem(cname) || this.getCookie(cname)) {
          return localStorage.getItem(cname) || this.getCookie(cname);
        }
        return '';
      },
      /*
  ...
  return { storageUtil: globalConfig.pushAPI.storageUtil };
}(globalConfig);
...
initAPI = function (globalConfig) {
  function initAPIValues() {
    window._pushcrewDebuggingQueue = window._pushcrewDebuggingQueue || [];
    window._pushcrewDebuggingQueue.push('Inside initAPIValues function');
    var subscriberId = globalConfig.pushAPI.storageUtil.getFromStorage('wingify_push_subscriber_id') || false;
    if (Notification.permission === 'denied') {
      subscriberId = -1;
    }
    globalConfig.pushcrew.subscriberId = subscriberId;
    window._pushcrewDebuggingQueue.push('initAPIValues function Exit. SubscriberId is: ' + subscriberId);
  }
  return { initAPIValues: initAPIValues };
}(globalConfig);
api = function ($pushcrew, globalConfig) {
  $pushcrew(function () {
    ...
    storageUtil = globalConfig.pushAPI.storageUtil;
    window._pushcrewDebuggingQueue = window._pushcrewDebuggingQueue || [];
    ...
    globalConfig._pushcrew.removeFromSegment = function (segmentName) {
      window._pushcrewDebuggingQueue.push('removeSubscriberToSegment execution begin');
      var subscriberId = storageUtil.getFromStorage('wingify_push_subscriber_id') || false;
      var promise = $pushcrew.Deferred();
      if (!subscriberId) {
        window._pushcrewDebuggingQueue.push('Subscriber Id not present while removing from segment - ' + segmentId);
        promise.reject('Invalid Subscriber Id');
      } else {
        var segmentInfo = {
          id: segmentId,
        ...
        $pushcrew.ajax({
          url: pushcrewConfig.servicesServerURL + '/rs',
          type: 'GET',
          headers: { 'Content-type': 'application/x-www-form-urlencoded' },
          data: segmentInfo,
          crossDomain: true,
          error: function () {
            window._pushcrewDebuggingQueue.push('remove from segment call failed');
            promise.reject();
          },
          success: function (response) {
            window._pushcrewDebuggingQueue.push('remove from segment call success - ' + segmentId);
            promise.resolve(true);
          }
        });
      }
      window._pushcrewDebuggingQueue.push('removeFromSegment execution exit');
      return promise;
    };
    globalConfig._pushcrew.noTrigger = function (value) {
      window._pushcrewDebuggingQueue.push('Updating value of no-trigger optIn: ' + value);
      globalConfig.pushcrew.noTrigger = value;
    };
    globalConfig._pushcrew.APIReady = function (callback) {
      window._pushcrewDebuggingQueue.push('Executing API ready callback');
      callback();
    };
    globalConfig._pushcrew.executeFailureQueue = function (reason) {
      window._pushcrewDebuggingQueue.push('Executing failure queue begin');
      for (var i = 0; i < globalConfig.apiFailureQueue.length; i++) {
        globalConfig.apiFailureQueue[i](reason);
      }
      globalConfig.apiFailureQueue = [];
    ...
    globalConfig._pushcrew.triggerOptOut = function () {
      if (globalConfig.pushcrew.isAPIReady === globalConfig.APIReadyState.UNINITIALIZED) {
        globalConfig._internalq.push(['triggerOptOut']);
        return;
      }
    ...
    globalConfig._pushcrew.addSubscriberToSegment = function (segment, callback) {
      ...
      if (segment.length >= 30 || !globalConfig._utils.validString(segment)) {
        window._pushcrewDebuggingQueue.push('SegmentName lenght is not proper');
        returnValue = false;
    ...
    globalConfig._pushcrew.removeSubscriberFromSegment = function (segment, callback) {
      ...
      if (callback) {
        window._pushcrewDebuggingQueue.push('Executing callback for removeSubscriberToSegment');
        callback(returnValue);
      }
      window._pushcrewDebuggingQueue.push('removeSubscriberToSegment execution complete');
    };
    ...
    globalConfig._pushcrew.showModal = function (modal, category) {
      ...
      if (category === 1) {
        textOfModal = modal.text.ALREADYSUBSCRIBED || 'You are already subscribed to get instant alerts from us. Keep an eye on your browser for new notifications from us.';
             ...
             modal.text = modal.text || 'Please click on ‘Allow’ in the opt-in box on top left of your browser screen to get instant alerts from our website. No email required.';
         }*/ else if (category === 3) {
      ...
      /*    var modalShown2 = '<div id="pushcrew-popup" class="pushcrew-modal-box">  '+
      '    <a href="#" class="js-pushcrew-modal-close pushcrew-close">×</a>'+
      '  <div class="pushcrew-modal-body">'+
      '<img src=' + pushcrewConfig.style.iconUrl + ' width=86/>'+
      '    <p style="margin-top:20px">'+modal.text+'</p>'
      '  </div>'+
      '</div>';*/
    ...
    globalConfig._pushcrew.subscribeToPush = function (segmentName, modal) {
      ...
      if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
        ...
        if (pushcrewConfig.style.brandingType === 1 || modal) {
          ...
          if (pushcrewConfig.pcBranding === 1) {
            httpsBrandingModal += '<div class="pushcrew-branding"><a href="https://pushcrew.com/?ref=' + pushcrewConfig.companyName + '" target="_blank" style="text-decoration: none;"><span>Notifications Powered By </span><img src="https://pushcrew.com/assets/images/pushcrew-white.png" width=85/></a></div>';
          }
      ...
      if (window.pctracker && Notification.permission === 'default') {
        // When the HTTPS Opt-In is shown
        window.pctracker.track(window.pctracker.supportedEvents.optInVisible);
      ...
      subscribePromise.then(function (newSubscription) {
        ...
        if (response !== 'error' && response !== 'denied') {
          if (window.pctracker) {
            // When Allow is clicked on HTTPS Opt-In
            window.pctracker.track(window.pctracker.supportedEvents.optInAllow);
          }
          window._pushcrewDebuggingQueue.push('Subscription done successfully');
          manageSubscription.syncSubscription(JSON.stringify(newSubscription), '', '', response.subscriptionId, response.endpoint, 'insert').then(function () {
            initAPI.initAPIValues();
            ...
            storageUtil.addToStorage('wingify_push_subscription_status', 'subscribed', 10000);
            if (window.pctracker) {
              // When successfully subscribed from HTTPS
              window.pctracker.track(window.pctracker.supportedEvents.subscriptionSuccessful);
            }
            if (segmentName !== '') {
          ...
          if (response === 'denied') {
            ...
            if (window.pctracker) {
              window.pctracker.track(window.pctracker.supportedEvents.optInBlock);
              window.pctracker.track(window.pctracker.supportedEvents.subscriptionBlocked);
            }
            ...
            if (window.pctracker && Notification.permission === 'default') {
              ...
              window.pctracker.track(window.pctracker.supportedEvents.optInClosed);
              /*
              ...
              if (window.pushcrewOptInVisibleTimestamp && window.pushcrewOptInClosedTimestamp && window.pushcrewOptInClosedTimestamp - window.pushcrewOptInVisibleTimestamp <= 500) {
                window.pctracker.track(window.pctracker.supportedEvents.optInBlockByBrowser);
              }
            }
            if (globalConfig._pushcrew.storedSubscriptionFailureCallback) {
              globalConfig._pushcrew.storedSubscriptionFailureCallback({
          ...
          }  /*if(pushcrew.subscriptionSuccessCallback){
                     pushcrew.subscriptionSuccessCallback(pushcrew.subscriberId);
             }*/
        }
      });
...
}(customizejQuery, globalConfig);

pushSubscription = function ($pushcrew, globalConfig) {
  globalConfig.pushAPI.pushSubscription = function () {
    ...
    return {
      ...
      */
      ...
      /*
      * Detects browser type(Chrome or Safari for now)
      */
      detectBrowserType: function () {
        if ('chrome' in window && 'serviceWorker' in navigator) {
          this.browserType = 'chrome';
        } else if (/Firefox[\/\s](\d+\.\d+)/.test(navigator.userAgent) && 'serviceWorker' in navigator) {
          this.browserType = 'firefox';
        } else if ('safari' in window && typeof window.safari === 'object' && 'pushNotification' in window.safari) {
      ...
      */
      ...
      detectLandingPage: function () {
        this.subscriptionPageUrl = window.location.href;
      },
      ...
      detectServiceWorkerPath: function () {
        if (this.browserType === 'chrome') {
          this.serviceWorkerPath = pushcrewConfig.swPath;
        } else if (this.browserType === 'firefox') {
          this.serviceWorkerPath = pushcrewConfig.firefoxSwPath;
        }
      }
    };
  }();
...
manageSubscription = function ($pushcrew, globalConfig, storageUtil, pushSubscription, jstz) {
  globalConfig.pushAPI.manageSubscription = function () {
    var syncServerURL = pushcrewConfig.serverSyncUrl;
    var safariServerURL = pushcrewConfig.serverSyncUrl;
    ...
    var manageSubscriptionAPI = {
      ...
      callSyncSubscription: function (data, promise, action, subscriptionId, subscriptionEndPoint) {
        var setPushSubscriptionStorage = function () {
            if (action !== 'unsubscribe-ui' && action !== 'unsubscribe-native') {
              globalConfig.pushAPI.storageUtil.addToStorage('wingify_push_subscription_id', subscriptionId, 10000);
              globalConfig.pushAPI.storageUtil.addToStorage('wingify_push_subscription_endpoint', subscriptionEndPoint, 10000);
            }
          }, removeWingifySegmentationKeys = function (uniqueSegmentStartString) {
            var allStorageItems = Object.keys(globalConfig.pushAPI.storageUtil.getAllStorageItemsWithMatchRegex(uniqueSegmentStartString)) || [], allCookies = Object.keys(globalConfig.pushAPI.storageUtil.getAllCookiesWithMatchRegex(uniqueSegmentStartString)) || [];
            allStorageItems.forEach(function (storageItem) {
              globalConfig.pushAPI.storageUtil.deleteFromStorage(storageItem);
            });
            allCookies.forEach(function (cookieItem) {
              globalConfig.pushAPI.storageUtil.deleteFromStorage(cookieItem);
            });
          }, _this = this;
        $pushcrew.ajax({
          ...
          dataType: 'jsonp',
          error: function () {
            globalConfig.pushAPI.storageUtil.addToStorage('wingify_push_db_status', 'false', 10000);
            setPushSubscriptionStorage();
            promise.resolve('error');
          },
          success: function (response) {
            if (response.status === 'success') {
              if (response.hasOwnProperty('subscriber_id')) {
                globalConfig.pushAPI.storageUtil.addToStorage('wingify_push_subscriber_id', response.subscriber_id, 10000);
                globalConfig.pushAPI.storageUtil.addToStorage('wingify_push_db_status', 'true', 10000);
                globalConfig.pushAPI.storageUtil.addToStorage('wingify_push_subscription_status', 'subscribed', 10000);
              }
              if (action === 'unsubscribe-ui' || action === 'unsubscribe-native') {
                globalConfig.pushAPI.storageUtil.addToStorage('wingify_push_subscription_status', 'unsubscribed', 10000);
                globalConfig.pushAPI.storageUtil.deleteFromStorage('wingify_push_subscriber_id');
                globalConfig.pushAPI.storageUtil.deleteFromStorage('wingify_push_db_status');
                globalConfig.pushAPI.storageUtil.deleteFromStorage('wingify_push_subscription_endpoint');
                globalConfig.pushAPI.storageUtil.deleteFromStorage('wingify_push_subscription_id');
                removeWingifySegmentationKeys('wingifySegment-');
                removeWingifySegmentationKeys('segment-evalDate-');
              }
              if (action === 'insert' || action === 'insert-failed') {
                _this.getUserProperties();
              }
              // When the permission is Blocked from Site Settings
              if (window.pctracker && Notification.permission === 'denied') {
      ...
      addToSegment: function (segmentId) {
        window._pushcrewDebuggingQueue.push('addSubscriberToSegment execution begin');
        var subscriberId = globalConfig.pushAPI.storageUtil.getFromStorage('wingify_push_subscriber_id') || false;
        var promise = $pushcrew.Deferred();
        if (!subscriberId) {
      ...
      getUserProperties: function () {
        ...
        var promise = $pushcrew.Deferred(), cookiesItemsOfSegmentationKeysLen = Object.keys(globalConfig.pushAPI.storageUtil.getAllCookiesWithMatchRegex('wingifySegment-') || {}).length;
        if (pushcrewConfig.segmentations && pushcrewConfig.segmentations.length === 0 || cookiesItemsOfSegmentationKeysLen === (pushcrewConfig.segmentations || []).length) {
          promise.resolve(true);
        } else {
          $pushcrew.ajax({
            url: pushcrewConfig.servicesServerURL + '/dyn',
            type: 'GET',
            dataType: 'jsonp',
            error: function () {
      ...
      updateSubscription: function (subscriptionDetails) {
        var promise = $pushcrew.Deferred();
        var subscription = this.splitSubscriptionParameters(subscriptionDetails);
        // get storage details for subscription data.
        var wingifyPushDbStatus = globalConfig.pushAPI.storageUtil.getFromStorage('wingify_push_db_status'), wingifyPushSubscriberId = globalConfig.pushAPI.storageUtil.getFromStorage('wingify_push_subscriber_id'), wingifyPushSubscriptionId = globalConfig.pushAPI.storageUtil.getFromStorage('wingify_push_subscription_id'), wingifyPushSubscriptionEndpoint = globalConfig.pushAPI.storageUtil.getFromStorage('wingify_push_subscription_endpoint');
        var subscriptionString = JSON.stringify(subscriptionDetails);
        ...
        if (wingifyPushDbStatus === 'false') {
          ...
          if (wingifyPushSubscriptionId === subscription.subscriptionId && wingifyPushSubscriberId === '') {
            //get subscriber id from db in case its cookie not present and if subscription id matches subscription id cookie
            this.syncSubscription(subscriptionString, '', '', subscription.subscriptionId, subscription.endpoint, 'get-subscriberid-from-server').then(function (response) {
              promise.resolve(response);
            });
          }
      ...
      splitSubscriptionParameters: function (subscriptionDetails) {
        if (subscriptionDetails !== 'error' && subscriptionDetails !== 'denied') {
          var endpointURL = '', endpoint = '';
          /*
           * Condition added while Firefox implementation for push api
           * Check for firefox else, proceed with default code
           */
      ...
      setNewPushSubscriber: function (subscriptionDetails, sendFirstNotification) {
        ...
        if (response !== 'error' && response !== 'denied') {
          this.syncSubscription(JSON.stringify(subscriptionDetails), '', '', response.subscriptionId, response.endpoint, 'insert').then(function (value) {
            globalConfig.pushAPI.storageUtil.addToStorage('wingify_push_subscription_status', 'subscribed', 10000);
      ...
      unsubscribeFromPushNotification: function () {
        ...
        var promiseInside = $pushcrew.Deferred();
        promise.then(function (response) {
          if (response !== 'error') {
            var subscriptionDetails = globalConfig.pushAPI.manageSubscription.splitSubscriptionParameters(response), wingifyPushSubscriberId = globalConfig.pushAPI.storageUtil.getFromStorage('wingify_push_subscriber_id'), wingifyPushSubscriptionId = globalConfig.pushAPI.storageUtil.getFromStorage('wingify_push_subscription_id');
      ...
      syncUnsubscribedUsers: function () {
        var promise = $pushcrew.Deferred();
        var wingifyPushSubscriberId = globalConfig.pushAPI.storageUtil.getFromStorage('wingify_push_subscriber_id'), wingifyPushSubscriptionId = globalConfig.pushAPI.storageUtil.getFromStorage('wingify_push_subscription_id');
        ...
        } else if (Notification.permission === 'default' && (wingifyPushSubscriberId !== '' || wingifyPushSubscriptionId !== '')) {
          this.syncSubscription('', wingifyPushSubscriberId, '', wingifyPushSubscriptionId, '', 'unsubscribe-native').then(function (value) {
            promise.resolve(value);
          });
        } else {
      ...
      getSafariDeviceToken: function (permissionData) {
        if (permissionData.permission === 'default') {
          window.safari.pushNotification.requestPermission(safariServerURL, globalConfig.pushAPI.pushSubscription.websiteSafariPushId, {}, this.getSafariDeviceToken);
        } else if (permissionData.permission === 'denied') {
          console.log('Safari Subscription Status - Denied');
        } else if (permissionData.permission === 'granted') {
          console.log('Safari Subscription Status - Granted');
        }
      },
    ...
    window.PC.evalSegments = function () {
      ...
      if (segments instanceof Array) {
        segments.map(function (segment) {
          var isAlreadyInSegment = globalConfig.pushAPI.storageUtil.getCookie('wingifySegment-' + segment.sid) || globalConfig.pushAPI.storageUtil.getFromStorage('wingifySegment-' + segment.sid) || false, isToBeAddedToSegment = false;
          ...
          if (!isAlreadyInSegment && isToBeAddedToSegment) {
            manageSubscriptionAPI.addToSegment(segment.sid).then(function () {
              var currentTime = new Date().toString();
              globalConfig.pushAPI.storageUtil.addToStorage('segment-evalDate-' + segment.sid, currentTime, 365);
              globalConfig.pushAPI.storageUtil.setCookie('segment-evalDate-' + segment.sid, currentTime, 365);
              globalConfig.pushAPI.storageUtil.addToStorage('wingifySegment-' + segment.sid, segment.vn, 365);
              globalConfig.pushAPI.storageUtil.setCookie('wingifySegment-' + segment.sid, segment.vn, 365);
            });
          }
        });
      }
...
}(jquery, globalConfig, storageUtil, pushSubscription, jstz);

httpChildMain = function (globalConfig, $pushcrew) {
  $pushcrew(document).ready(function () {
    // Variables to be used for push notifications.
    var storageUtil = globalConfig.pushAPI.storageUtil, manageSubscription = globalConfig.pushAPI.manageSubscription, pushSubscription = globalConfig.pushAPI.pushSubscription, timestamp;
    window._pushcrewDebuggingQueue = window._pushcrewDebuggingQueue || [];
    // if customer add the details in the DB and send new subscription details to GCM.
    function initPushNotifications() {
      window._pushcrewDebuggingQueue.push('HTTPSChildWindow Executing initPushNotifications function begin');
      var promise = pushSubscription.getUserSubscription();
      promise.then(function (response) {
        if (response === false) {
          if (pushSubscription.canPushBeEnabled === true) {
            var subscribePromise = pushSubscription.subscribe();
            $pushcrew('#js-spinner').addClass('hide');
            $pushcrew('#js-text').removeClass('hide');
            subscribePromise.then(function (newSubscription) {
              if (newSubscription === 'denied') {
                window._pushcrewDebuggingQueue.push('HTTPSChildWindow Executing initPushNotifications function, user has denied for notifications');
                storageUtil.addToStorage('wingify_push_subscription_status', 'blocked', 10000);
                setTimeout(function () {
                  // Setting value of window.isBlockedUser if the user has blocked push notifications
                  window.isBlockedUser = true;
                  sendCookiestoParent(true);
                }, 500);
              } else {
                if (newSubscription === 'error') {
                  window._pushcrewDebuggingQueue.push('HTTPSChildWindow Executing initPushNotifications function, user has closed notifications');
                }
                manageSubscription.doesUserResponseExists = true;
                manageSubscription.setNewPushSubscriber(newSubscription, false).then(function () {
                  setTimeout(function () {
        ...
        }  // if existing user, updated the details.
        else {
          manageSubscription.doesUserResponseExists = true;
      ...
      window._pushcrewDebuggingQueue.push('HTTPSChildWindow Executing initPushNotifications function complete');
    }
    function getChromeVersion() {
      var raw = navigator.userAgent.match(/Chrom(e|ium)\/([0-9]+)\./);
      return raw ? parseInt(raw[2], 10) : false;
    }
    // Send the cookie values to parent give postMessage API.
    function sendCookiestoParent(closeWindow) {
      window._pushcrewDebuggingQueue.push('HTTPSChildWindow Executing sendCookiestoParent function begin');
      if (closeWindow) {
        window._pushcrewDebuggingQueue.push('HTTPSChildWindow Executing sendCookiestoParent function complete. Directly close the window');
        window.close();
        return;
      }
      if (window.opener) {
        var cookieObject = {
          'wingify_push_db_status': storageUtil.getFromStorage('wingify_push_db_status'),
          'wingify_push_subscriber_id': storageUtil.getFromStorage('wingify_push_subscriber_id'),
          'wingify_push_subscription_id': storageUtil.getFromStorage('wingify_push_subscription_id'),
          'wingify_push_subscription_status': storageUtil.getFromStorage('wingify_push_subscription_status')
        };
        if (window.isNewUser) {
          cookieObject.is_new_user = true;
        } else {
          cookieObject.is_new_user = false;
        }
        // Set the cookie for blocked user
        cookieObject.is_blocked_user = window.isBlockedUser ? true : false;
        var storageItemsOfSegmentation = storageUtil.getAllStorageItemsWithMatchRegex('wingifySegment-') || {}, storageItemsOfSegmentationKeys = Object.keys(storageItemsOfSegmentation), cookiesItemsOfSegmentation = storageUtil.getAllCookiesWithMatchRegex('wingifySegment-') || {}, cookiesItemsOfSegmentationKeys = Object.keys(cookiesItemsOfSegmentation), dateStorageItemsOfSegmentation = storageUtil.getAllStorageItemsWithMatchRegex('segment-evalDate-') || {}, dateStorageItemsOfSegmentationKeys = Object.keys(dateStorageItemsOfSegmentation), evalDateItemsOfSegmentation = storageUtil.getAllCookiesWithMatchRegex('segment-evalDate-') || {}, evalDateItemsOfSegmentationKeys = Object.keys(evalDateItemsOfSegmentation);
        storageItemsOfSegmentationKeys.forEach(function (segmentkey) {
          cookieObject[segmentkey] = storageUtil.getFromStorage(segmentkey);
        });
        cookiesItemsOfSegmentationKeys.forEach(function (segmentkey) {
          cookieObject[segmentkey] = cookieObject[segmentkey] || storageUtil.getFromStorage(segmentkey);
        });
        dateStorageItemsOfSegmentationKeys.forEach(function (segmentkey) {
          cookieObject[segmentkey] = storageUtil.getFromStorage(segmentkey);
        });
        evalDateItemsOfSegmentationKeys.forEach(function (segmentkey) {
          cookieObject[segmentkey] = cookieObject[segmentkey] || storageUtil.getFromStorage(segmentkey);
        });
        window.opener.postMessage(cookieObject, '*');
        window._pushcrewDebuggingQueue.push('HTTPSChildWindow Executing sendCookiestoParent function, Post message send');
      }
      window._pushcrewDebuggingQueue.push('HTTPSChildWindow Executing sendCookiestoParent function complete');
    }
    function optOut() {
      var html = '<div><img src="https://pushcrew.com/assets/images/opt-out.png"/> <br /> <p>Please click on the lock icon in this window as mentioned in the image above to unsubscribe from push notifications.</p></div>';
      $pushcrew('.subscribe-helper-text').empty().append(html).css({ 'margin-top': 0 });
      window._pushcrewDebuggingQueue.push('HTTPSChildWindow optOut image has been added on page');  /*if(window.opener) {
                                                                                                        window.opener.postMessage('unsubscribe','*');
                                                                                                    }*/
    }
    function unblock() {
      var html = '<div><img src="https://pushcrew.com/assets/images/unblock.png" /> <br /> <p>You seem to have previously blocked/denied push notifications from ' + pushcrewConfig.companyName + '. See the image above to know how to unblock them.</p></div>';
      $pushcrew('.subscribe-helper-text').empty().append(html).css({ 'margin-top': 0 });
      window._pushcrewDebuggingQueue.push('HTTPSChildWindow unblock image has been added on page');
    }
    detectPrivateMode.detectPrivateMode(function (is_private) {
      window._pushcrewDebuggingQueue.push('HTTPSChildWindow detectPrivateMode callback function begin');
      if (is_private) {
        window._pushcrewDebuggingQueue.push('HTTPSChildWindow detectPrivateMode callback, page is opened in incognito mode');
        $pushcrew('#js-logo-customer').attr('src', pushcrewConfig.style.iconUrl);
        $pushcrew('#js-user-customize-text').text('You seem to be browsing in Incognito mode. The website can\'t get your permission to opt-in to push notifications in the Incognito browser. Please turn off the incognito mode Chrome browser and try again.');
      } else {
        timestamp = Date.now();
        $pushcrew('head').append('<link rel="manifest" href="' + pushcrewConfig.manifestUrl + '?t=' + timestamp + '">');
        if (pushcrewConfig.pcBranding === 0) {
          window._pushcrewDebuggingQueue.push('HTTPSChildWindow detectPrivateMode callback function, pcBranding hide');
          $pushcrew('footer').hide();
        }
        $pushcrew('.anchor_pushcrew').attr('href', 'https://pushcrew.com/?ref=' + pushcrewConfig.subdomain);
        if (pushcrewConfig.style.allowTextChildWindow) {
          $pushcrew('#js-user-customize-text').text(pushcrewConfig.style.allowTextChildWindow);
        }
        $pushcrew('#js-offer-text').html(pushcrewConfig.style.offerTextChildWindow);
        $pushcrew('#js-logo-customer').attr('src', pushcrewConfig.style.iconUrl);
        $pushcrew(document).prop('title', 'Subscribe to notifications from ' + pushcrewConfig.companyName);
        // initialize the browser for push subscriptions.
        storageUtil.initialiseState('self');
        ...
        //added a call to detect browser type
        //These functions will get extra data about subscriber
        pushSubscription.detectUserLanguage();
        pushSubscription.detectLandingPage();
        pushSubscription.detectDomain();
        pushSubscription.detectServiceWorkerPath();
        if (window.location.search.includes('isAPIcall=true')) {
          globalConfig.pushcrew.isAPIcall = true;
        } else {
          globalConfig.pushcrew.isAPIcall = false;
        }
        if ('chrome' in window && 'serviceWorker' in navigator && getChromeVersion() >= 42) {
          //assign service worker file for chrome/default
          pushSubscription.serviceWorkerPath = pushcrewConfig.swPath;
        ...
        if ((Notification.permission === 'granted' || Notification.permission === 'default') && (storageUtil.getFromStorage('wingify_push_subscription_status') === 'blocked' || storageUtil.getFromStorage('wingify_push_subscription_status') === 'unsubscribed')) {
          storageUtil.addToStorage('wingify_push_subscription_status', '', 10000);
        }
        if (Notification.permission === 'denied') {
          storageUtil.addToStorage('wingify_push_subscription_status', 'blocked', 10000);  /*setTimeout(function(){
                                                                                               sendCookiestoParent(false);
                                                                                           }, 500);*/
        }
        // if user is already subscribed, show ways to unsubscribe.
        // else, store user details for new subscription.
        var isChromeSupported = 'chrome' in window && 'serviceWorker' in navigator && getChromeVersion() >= 42;
        var isFirefoxSupported = /Firefox[\/\s](\d+\.\d+)/.test(navigator.userAgent) && 'serviceWorker' in navigator;
        if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
          isFirefoxSupported = false;
        }
        if (!isChromeSupported && !isFirefoxSupported) {
          $pushcrew('#js-text').html('Push notifications don\'t seem to be enabled for this browser. Please use Chrome(version 42 or above) or Firefox(version 44 or above)to subscribe for push notifications by ' + pushcrewConfig.companyName + '.');
        } else if (window.location.search.includes('?action=subscribe') || window.location.search.includes('?action=unblock')) {
          if (storageUtil.getFromStorage('wingify_push_subscription_status') === 'subscribed') {
            $pushcrew('#js-text').html('You seem to have already been subscribed to push notifications from ' + pushcrewConfig.companyName + '. You can close this window.');
          } else if (storageUtil.getFromStorage('wingify_push_subscription_status') === '') {
            $pushcrew('#js-spinner').removeClass('hide');
            $pushcrew('#js-text').addClass('hide');
            initPushNotifications();
          } else if (storageUtil.getFromStorage('wingify_push_subscription_status') === 'unsubscribed' || storageUtil.getFromStorage('wingify_push_subscription_status') === 'blocked') {
            unblock();
          }
        } else if (window.location.search.includes('?action=triggerOptOut')) {
          if (storageUtil.getFromStorage('wingify_push_subscription_status') === 'subscribed') {
            optOut();
          } else {
            $pushcrew('#js-text').html('You don\'t seem to have push notifications enabled from ' + pushcrewConfig.companyName + '. You can close this window.');
          }
        } else if (window.location.search.includes('?action=unsubscribe')) {
          $pushcrew('#js-text').html('Please wait unsubscribing you from ' + pushcrewConfig.companyName);
          manageSubscription.unsubscribeFromPushNotification().then(function () {
            $pushcrew('#js-text').html('You have been unsubscribed from ' + pushcrewConfig.companyName + '. You can close this window.');
          });
        }
        $pushcrew(window).bind('beforeunload', function () {
          sendCookiestoParent(false);
        });
        window._pushcrewDebuggingQueue.push('HTTPSChildWindow detectPrivateMode callback function exit');
      }
    });
  });
}(globalConfig, jquery);
vwoUtils_cookies_CookiesEventEnum = {
  SET_COOKIE: 'sC',
  GET_COOKIE: 'gC',
  ERASE_COOKIE: 'eC',
...
vwoUtils_triggerEvent = function () {
  /* jshint camelcase:false */
  window._vwo_evq = window._vwo_evq || [];
  var JSLIB_INIT = 'jI', eventQueue = window._vwo_evq;
  /**
   * Push the event to _vwo_evq
   */
  window._vwo_ev = window._vwo_ev || function () {
    if (arguments[0] === JSLIB_INIT) {
      // JSLIB_INIT must be dispatched before every other event
      // hack :(
      eventQueue.unshift([JSLIB_INIT]);
      return;
    }
    eventQueue.push([].slice.call(arguments));
  };
  VWO._.triggerEvent = window._vwo_ev;
  return window._vwo_ev;  /* jshint camelcase:true */
}();
vwoUtils_tld = function (triggerEvent) {
  var TldEventEnum = { PARSE_TLD: 'pTLD' }, domainList = [
      'co',
      'org',
      'com',
      'net',
   ...
   * Strips the protocol and subdomain of the URL.
   */
  return function (url) {
    var retval, domainPieces = url.split('.'), length = domainPieces.length, piece = domainPieces[length - 2];
    // find the tld of this url. then find the actal domain
    if (piece && -1 !== $.inArray(piece, domainList)) {
      retval = domainPieces[length - 3] + '.' + piece + '.' + domainPieces[length - 1];
      // event params: originalURL, strippedURL
      triggerEvent(TldEventEnum.PARSE_TLD, url, retval);
      return retval;
    }
    retval = piece + '.' + domainPieces[length - 1];
    // event params: originalURL, strippedURL
    triggerEvent(TldEventEnum.PARSE_TLD, url, retval);
    return retval;
  };
}(vwoUtils_triggerEvent);
vwoUtils_cookies_cookies = function (CookiesEventEnum, tld, triggerEvent) {
  /* jshint camelcase:false */
  var cookieDays = window._vis_opt_cookieDays, visDebug = window._vis_debug, visOptDomain = window._vis_opt_domain, vwoCookieDomain = window._vwo_cookieDomain, vwoExp = window._vwo_exp || {}, accountId = window._vwo_acc_id,
  ...
  var cookies = {
     ...
     * @param {number|boolean} days Expiry duration of the cookie in days. Overridden by _vis_opt_cookieDays if set.
     * @param {string} domain Domain name on which cookie to be made, Default: Top level domain (optional)
     * @param {timestamp} expiryTs Exact expiry timestamp of the cookie, if days has not been given
     * Creates a cookie with encoded value.
     */
    _create: function (name, value, days, domain, expiryTs) {
      if (visDebug && name.indexOf('debug') !== 0) {
        name = 'debug' + name;
      }
      if (!isNaN(cookieDays = parseFloat(cookieDays)) && isFinite(cookieDays)) {
        days = cookieDays;
      }
      var expires = '', date;
      if (expiryTs) {
        expires += '; expires=' + function () {
          date = new Date(expiryTs);
          return date.toGMTString();
        }();
      } else {
        if (days) {
          expires += '; expires=' + function () {
            date = new Date(new Date().getTime() + days * 86400000);
            return date.toGMTString();
          }();
        } else if (days === false) {
      ...
      if (!domain) {
        domain = cookies.domain || tld(document.URL);
      }
      domain = '; domain=.' + domain;
      document.cookie = name + '=' + encodeURIComponent(value) + expires + domain + '; path=/';
    },
    /**
     * Create Cookie using _create function
     * @param name
     * @param value
     * @param days
     * @param domain
     * @param expiryTs
     */
    create: function (name, value, days, domain, expiryTs) {
      this._create(name, value, days, domain, expiryTs);
      // params: cookieName, cookieValue, daysUntilExpiry, isThirdParty
      triggerEvent(CookiesEventEnum.SET_COOKIE, name, value, days, expiryTs);
    },
    /**
     * @param {string} name Name of the cookie.
     * @return {string|null} decoded value of the cookie if found, else null.
     * Returns the decoded value of the cookie with the name or null if it doesn't exists.
     */
    get: function (name) {
      if (visDebug) {
        name = 'debug' + name;
    ...
    erase: function (name, domain) {
      //passing expiryTS as 1 for setting expiry as Jan 1, 1970
      this.create(name, '', false, domain, 1);
      // Trigger Event, params: cookieName
      triggerEvent(CookiesEventEnum.ERASE_COOKIE, name);
    },
    /**
     * @param {string} name Name of the cookie.
     * @param {string} value Value of the cookie.
     * @param {number=} days Expiry duration of the cookie in days. Overridden by _cookie_days if set.
     * @param {string} domain Domain name on which cookie to be made, Default: Top level domain (optional)
     * @param {string} experimentId Experiment id to check if third party cookie required or not TODO: We can remove it
     * Creates a third party cookie with encoded value by sending a req to create cookie(3rd party on dev domain)
     * specific to that experiment(i.e. with exp id in cookie name)
     * No, stray form or frame tag is left in code (any form or frame inserted for 3rd party cookie creation is removed)
     */
    createThirdParty: function (name, value, days, domain, experimentId) {
      var frame, url, form, $;
      this._create(name, value, days, domain);
      if (visDebug && name.indexOf('debug') !== 0) {
        name = 'debug' + name;
      }
      /* jshint camelcase:false */
      $ = window.vwo_$ || window.$;
      // jQuery is required to create the third party cookie
      if (!$ || !experimentId || !vwoExp[experimentId].multiple_domains) {
        triggerEvent(CookiesEventEnum.SET_THIRD_PARTY_COOKIE_ERROR, name, value, days, domain);
        return;
      }
      /* jshint camelcase:true */
      frame = $('<iframe>').attr({
        ...
        // On load of frame remove the frame as its job is done
        if (name.indexOf('split') !== -1) {
          this.parentNode.removeChild(this);
        }
      });
      url = '//dev.visualwebsiteoptimizer.com/ping_tpc.php?account=' + accountId + '&name=' + encodeURIComponent(name) + '&value=' + encodeURIComponent(value) + '&days=' + days + '&random=' + Math.random();
      if (/MSIE (\d+\.\d+);/.test(navigator.userAgent)) {
        //if < IE 7
        frame.attr('src', url);
      } else {
        //Changelog_1: new form submitted everytime and removed cause using a single form to submit to same frame was cancelling previous reqs.
        form = $('<form>').attr({
          'action': '//dev.visualwebsiteoptimizer.com/ping_tpc.php',
          'accept-charset': 'UTF-8',
          'target': frame.attr('name'),
      ...
      // params: cookieName, cookieValue, daysUntilExpiry, experimentId, isThirdParty
      triggerEvent(CookiesEventEnum.SET_COOKIE, name, value, days, experimentId, true);
    }
  };
  /**
   *@type {string}
   * Custom Domain(if available) otherwise calculated top level domain
   **/
  _domain = visOptDomain || vwoCookieDomain || tld(location.host);
  cookies.domain = _domain;
  VWO._.cookies = cookies;
  return cookies;
}(vwoUtils_cookies_CookiesEventEnum, vwoUtils_tld, vwoUtils_triggerEvent);
vwoUtils_referrer = function (cookies) {
  var actualReferrer = cookies.get('_vwo_referrer');
  cookies.create('_vwo_referrer', 0, -365);
  if (typeof actualReferrer !== 'string') {
  ...
  return {
    ...
    set: function () {
      cookies.create('_vwo_referrer', actualReferrer, 0.00018);  //expiry of 15 secs
    }
  };
}(vwoUtils_cookies_cookies);
vwoUtils_segmentation = function (referrer) {
  /**
   * @const
   **/
  var undef = 'undefined',
     ...
     **/
    parseBase = 10, visitorInformation = window.VWO && window.VWO.data && window.VWO.data.vi;
  /**
  ...
  return function () {
    ...
    var D = new Date(),
      //Date
      N = navigator, d = document, n = N.userAgent,
      //n = userAgent
      v = N.vendor,
      //v = navigator.vendor
      a = d.createElement('a'),
      //New Element Creation
      nl = n.toLowerCase(), aV = N.appVersion, bs = [
        //List of Browsers
        ...
        //Move IE above Chrome as Microsoft Edge contains Chrome in the UA
        {
          s: n,
          sS: 'MSIE',
          p: /(?:Trident\/.*?rv:|Windows NT.*?Edge\/)(?:[0-9]+[.0-9]*)/i.test(n),
          i: 'Explorer'
        },
        {
          s: n,
    ...
    return {
      ...
      k: function () {
        ...
        if (this.ors()) {
          var regex = new RegExp('[\\?&]' + or[this.ors() - 1].p + '=([^&#]*)'), results = regex.exec(this.R());
          if (null !== results) {
      ...
      gC: function (x) {
        ...
        if (0 < d.cookie.length) {
          var cs = d.cookie.indexOf(x + '='), ce;
          if (-1 !== cs) {
            cs = cs + x.length + 1;
            ce = d.cookie.indexOf(';', cs);
            if (-1 === ce) {
              ce = d.cookie.length;
            }
            return decodeURIComponent(d.cookie.substring(cs, ce));
          }
        }
        return '';
      },
      ...
      de: function () {
        ...
        if (nl.indexOf('ipod') > -1) {
          return 'ipod';
        }
        if (nl.indexOf('ipad') > -1 && this.wk()) {
          return 'ipad';
        }
        if (nl.indexOf('iphone') > -1) {
          return 'iphone';
        }
        if (nl.indexOf('android') > -1) {
          return 'android';
        }
        if (nl.indexOf('googletv') > -1) {
          return 'googletv';
        }
        if (nl.indexOf('symbian') > -1 || /series\s*[4-9]0/i.test(nl)) {
          return 'symbian';
        }
        if (nl.indexOf('blackberry') > -1 || nl.indexOf('vnd.rim') > -1 || nl.indexOf('bb10') > -1) {
          return 'blackberry';
        }
        if (nl.indexOf('windows phone') > -1) {
          return 'winphone';
        }
        return '';
      },
...
src_httpChild_without_tracker = function (detectPrivateMode, globalConfig, storageUtil, api, pushSubscription, manageSubscription, httpChildMain, segmentation) {
  window._pushcrewDebuggingQueue = window._pushcrewDebuggingQueue || [];
  window._vwo_s = segmentation || {};
  window._pcq = window._pcq || [];
  var storageUtility = storageUtil.storageUtil, subscriberId = storageUtility.getFromStorage('wingify_push_subscriber_id') || false, manageSubscriptionAPI = manageSubscription && manageSubscription.manageSubscription;
  if (subscriberId && subscriberId !== -1) {
    manageSubscriptionAPI.getUserProperties();
  }
}(detectPrivateMode, globalConfig, storageUtil, api, pushSubscription, manageSubscription, httpChildMain, vwoUtils_segmentation);
}());
})();